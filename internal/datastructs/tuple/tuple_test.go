...
func TestInternalTuple_New(t *testing.T) {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if len(tup.vars) != len(scenario.data) {
				t.Errorf("Expected len(tup.vars) to be %d. Got %d", len(scenario.data), len(tup.vars))
			}

			if !slices.Equal(scenario.data, tup.vars) {
				t.Error("Expected tup.vars to be equal to scenario.data")
			}
		})
	}
}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if tup.Len() != len(scenario.data) {
				t.Errorf("Expected tup.Len() to be %d. Got %d", len(scenario.data), len(tup.vars))
			}
		})
	}
}
